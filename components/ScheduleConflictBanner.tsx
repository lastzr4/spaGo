...
export default function ScheduleConflictBanner({ token, conflicts }: { token: string; conflicts: ScheduleConflict[] }) {
  ...
  return (
    <Link
      ...
      className="card-tap mb-5 flex animate-fade-in flex-col gap-2.5 rounded-2xl border border-red-500/30 bg-red-500/15 p-4"
    >
      <div className="flex items-center gap-2.5">
        ...
        <p className="flex-1 text-sm font-bold text-red-400">
      ...
      <div className="flex flex-col gap-1.5 pl-11">
        ...
        {conflicts.length > 3 && <p className="text-xs font-semibold text-red-400">+{conflicts.length - 3} lagi</p>}
      </div>
    </Link>
  );
}
