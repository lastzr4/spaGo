...
// Canvas UI — Liquid (https://canvasui.dev/docs/components/liquid)
// Installed manually (source copied from the docs page) rather than via
// `npx shadcn@latest add @canvas-ui/liquid-react`, since this project has no
// shadcn CLI/registry set up and the docs explicitly support manual install:
// "Open the component page ... copy the file into your project, and import it."
//
// One deliberate change from the upstream source: the inner content wrapper
// normally uses `height: 100%; overflow: auto`, which requires the parent
// <Liquid> to be given a fixed pixel height (their own demo does
// `style={{ height: 480 }}`). SpaGo's hero text length varies (admin-editable
// heroTitle/heroSubtitle), so a hard-coded height risks clipping or a
// scrollbar. Changed to `minHeight: 100%; overflow: visible` so content can
// grow past the given height instead of being cut off. Everything else is
