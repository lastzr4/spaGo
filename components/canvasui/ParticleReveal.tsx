...
// Canvas UI — Particle Reveal (https://canvasui.dev/docs/components/particle-reveal)
// Installed manually, same approach as Liquid.tsx in this folder: no shadcn
// CLI configured in this project, and the docs support copying the source
// directly. Same content-wrapper tweak as Liquid.tsx too — changed
// `height: 100%; overflow: auto` to `minHeight: 100%; overflow: visible` so
// the wrapped form (which can grow taller, e.g. validation errors) isn't
// clipped or scrollable. Everything else is unmodified upstream code.
import {
  useEffect,
  useRef,
  useState,
...
void main () {
  ...
  if (pc.x > uMaxX * uRes.x) { outColor = vec4(0.0); return; }
  if (uCrisp > 0.5) { outColor = samp(pc); return; }
  ...
  float ring = smoothstep(inner, radius, dist) * (1.0 - smoothstep(radius, radius + band, dist)) * uActive;
