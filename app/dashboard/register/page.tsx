...
export default function TherapistRegisterPage() {
  ...
  return (
    <main className="flex flex-1 flex-col px-6 py-8">
      <Link href="/" className="mb-4 text-sm text-brand-600">&larr; Balik</Link>
      <h1 className="mb-1 text-xl font-semibold text-brand-900">Daftar sebagai terapis</h1>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-5">
        ...
        <div>
          <label className="mb-2 block text-sm font-medium text-brand-900">Jantina anda</label>
          <div className="grid grid-cols-2 gap-3">
            <button type="button" onClick={() => setGender("FEMALE")} className={`rounded-xl border px-4 py-3 font-medium ${gender === "FEMALE" ? "border-brand-600 bg-brand-50 text-brand-700" : "border-black/10 bg-white text-gray-600"}`}>Wanita</button>
            <button type="button" onClick={() => setGender("MALE")} className={`rounded-xl border px-4 py-3 font-medium ${gender === "MALE" ? "border-brand-600 bg-brand-50 text-brand-700" : "border-black/10 bg-white text-gray-600"}`}>Lelaki</button>
          </div>
        </div>
        ...
        <div>
          ...
          <p className="mt-1 text-xs text-gray-400">Nota: pelanggan lelaki hanya akan dipadankan dengan terapis lelaki (polisi keselamatan).</p>
        </div>
          ...
          <div className="flex flex-wrap gap-2">
            {AREAS.map((a) => (
              <button
                ...
                className={`rounded-full border px-3 py-1.5 text-sm ${areas.includes(a) ? "border-brand-600 bg-brand-50 text-brand-700" : "border-black/10 bg-white text-gray-600"}`}
              >
                {a}
              </button>
            ))}
